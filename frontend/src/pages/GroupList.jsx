...
export default function GroupList() {
  ...
  // ─── Socket ───────────────────────────────────────────────────────────────
  const handleGroupNew = useCallback((group) => {
  ...
  // Khi accept invite → nhóm mới xuất hiện trong list
  ...
  }, [user?._id, handleGroupNew, handleGroupDeleted, handleGroupJoined]);
  // ─────────────────────────────────────────────────────────────────────────

  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      if (file.size > 5 * 1024 * 1024) { toast.error("Vui lòng chọn ảnh nhỏ hơn 5MB"); return; }
      setAvatarFile(file);
              ...
              >
                ...
                <div className="px-5 pb-5 pt-0 flex-1 flex flex-col items-center -mt-10 relative">
                  <div className="w-[72px] h-[72px] rounded-full border-4 border-white bg-slate-50 shadow-md flex items-center justify-center overflow-hidden z-10 transition-transform duration-300 group-hover/card:scale-110">
                    {group.avatar ? (
                      <img src={getFullUrl(group.avatar)} alt={group.name} className="w-full h-full object-cover" loading="lazy" onError={(e) => { e.target.style.display = "none"; e.target.parentNode.innerHTML = "👥"; }} />
                    ) : (
                  ...
                  <div className="mt-5 w-full flex items-center justify-between border-t border-slate-100 pt-4">
                    <div className="flex -space-x-2">
                      {group.members?.slice(0, 4).map((m) => {
                        ...
                        return (
                          <div key={m.user?._id} className="w-7 h-7 rounded-full border-2 border-white bg-gradient-to-tr from-indigo-500 to-purple-500 text-white flex items-center justify-center text-[10px] font-bold shadow-sm overflow-hidden" title={m.user?.name}>
                            {url ? <img src={getFullUrl(url)} alt="avt" className="w-full h-full object-cover" /> : m.user?.name?.charAt(0)?.toUpperCase()}
                          </div>
                        );
                      })}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50/50">
              ...
              <button onClick={closeModal} className="text-slate-400 hover:text-slate-700 bg-white shadow-sm border border-slate-100 w-8 h-8 flex items-center justify-center rounded-full transition-colors">✕</button>
            </div>

            <form onSubmit={handleCreate} className="p-6">
              <div className="flex flex-col items-center mb-6">
                <div onClick={() => fileInputRef.current?.click()} className="w-24 h-24 rounded-full border-2 border-dashed border-indigo-300 bg-indigo-50/50 flex flex-col items-center justify-center cursor-pointer hover:bg-indigo-50 transition-colors overflow-hidden relative group">
                  {avatarPreview ? (
              ...
              <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
                <button type="button" onClick={closeModal} className="px-5 py-2.5 rounded-xl font-bold text-slate-600 hover:bg-slate-100 transition-colors">Hủy</button>
