...
function SortableTaskItem({ task, currentUserId, onEdit, onDelete, onToggle }) {
  ...
  const style = {
    ...
    transition,
    opacity: isDragging ? 0 : 1, // Hide the original spot when dragging
    position: "relative",
  };
...
function Column({ id, title, tasks, currentUserId, onEdit, onDelete, onToggle }) {
  ...
  return (
    <div 
      ref={setNodeRef}
      className={`bg-slate-100/60 backdrop-blur-sm rounded-3xl p-4 flex flex-col w-full min-h-[500px] border transition-all duration-200 ${isOver ? 'border-indigo-400 bg-indigo-50/70 shadow-inner' : 'border-slate-200 shadow-sm'}`}
    >
...
export default function TaskBoard({ tasks, currentUserId, onStatusChange, onEdit, onDelete, onToggle }) {
  ...
  // Constraint distance to 5 so clicking buttons inside card doesn't trigger drag
  ...
  const handleDragEnd = (e) => {
    ...
    // Call callback to perform Optimistic update AND API call mapping!
